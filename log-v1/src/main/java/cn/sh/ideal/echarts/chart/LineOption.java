...
import cn.sh.ideal.echarts.series.MarkLineX;

public class LineOption extends BaseChartOption implements OptionGenerator {
	...
	public Option getOption() throws Exception {
		...
		line.symbolSize(6);
		line.markLine(new MarkLineX().silent(false).data(new PointData().type(MarkType.average).name("平均值")));
